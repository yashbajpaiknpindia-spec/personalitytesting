...
async function main() {
  ...
  console.log(`Seed check: ${templates.length} templates, admin account, AdminSettings...`)

  // Fast path for normal Render deploys: once the DB is already seeded, avoid
  // 48 upserts + bcrypt hashing on every build. FORCE_SEED=true keeps the old
  // full-refresh behaviour available when required.
  const [existingTemplateCount, existingAdmin, existingSettings] = await Promise.all([
    prisma.template.count({ where: { slug: { in: templateSlugs } } }),
    prisma.user.findUnique({ where: { phone: '917897671348' }, select: { id: true, role: true, plan: true, password: true } }),
    (prisma as any).adminSettings.findUnique({ where: { id: 'singleton' }, select: { id: true } }).catch(() => null),
  ])

  if (!forceSeed && existingTemplateCount >= templates.length) {
    console.log(`Templates already seeded (${existingTemplateCount}/${templates.length}) — skipping template upserts.`)
  } else {
    console.log(`Seeding ${templates.length} templates...`)
    const ops = templates.map(template => prisma.template.upsert({
      where: { slug: template.slug },
      update: template,
      create: template,
    }))
    // Transaction keeps the first seed consistent while still being faster than
    // 48 separate awaited network round-trips.
    await prisma.$transaction(ops)
    console.log('Templates seeded.')
  }

  // ── Seed hardcoded admin account (phone-based auth) ───────────────────
  const ADMIN_PHONE    = '917897671348'   // +91 7897671348
  const ADMIN_PASSWORD = 'Jaysiyaram@12'

  if (!forceSeed && existingAdmin?.role === 'ADMIN' && existingAdmin?.plan === 'TEAM' && existingAdmin?.password) {
    console.log(`Admin account already ready: phone ${ADMIN_PHONE} (role: ${existingAdmin.role})`)
  } else {
    ...
    console.log(`Admin account ready: phone ${ADMIN_PHONE} (role: ${admin.role})`)
  }

  // ── Seed AdminSettings singleton ──────────────────────────────────────
  if (!forceSeed && existingSettings?.id) {
    console.log('AdminSettings singleton already ready — skipping upsert.')
  } else {
    await (prisma as any).adminSettings.upsert({
      where: { id: 'singleton' },
