...
const FROM = process.env.RESEND_FROM_EMAIL || 'brandsyndicateindia@gmail.com'
...
export async function sendPasswordResetEmail(to: string, name: string, resetUrl: string) {
  await getResend().emails.send({
    from: FROM,
    to,
    subject: 'Reset your Brand Syndicate password',
    html: baseTemplate(
      `Reset your <em>password</em>.`,
      `Hi ${name || 'there'},<br><br>We received a request to reset your Brand Syndicate account password. Use the secure link below to create a new password. This link is valid for 30 minutes.<br><br>If you did not request this, you can safely ignore this email.`,
      'Reset Password',
      resetUrl
    ),
  })
