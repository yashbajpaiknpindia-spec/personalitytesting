...
const FROM = process.env.RESEND_FROM_EMAIL || 'studio@brandsyndicate.co'
...
body{background:#09090A;color:#E8E2D8;font-family:'Helvetica Neue',Arial,sans-serif;-webkit-font-smoothing:antialiased}
.wrap{max-width:560px;margin:0 auto;padding:48px 24px}
.logo{font-size:14px;letter-spacing:0.2em;text-transform:uppercase;color:#F0EAE0;margin-bottom:40px}
.logo span{color:#C9A84C}
.title{font-size:28px;line-height:1.2;color:#F0EAE0;margin-bottom:16px;font-weight:400}
.title em{color:#C9A84C;font-style:italic}
.body{font-size:14px;color:#5C564E;line-height:1.8;font-weight:300;margin-bottom:32px}
...
.footer{font-size:10px;color:#3A3530;letter-spacing:0.06em;border-top:1px solid rgba(255,255,255,0.06);padding-top:24px}
...
export async function sendWelcomeEmail(to: string, name: string) {
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://brandsyndicate.co'
