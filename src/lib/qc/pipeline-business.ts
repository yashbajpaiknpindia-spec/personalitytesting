...
// Business-mode QC pipeline — supported outputs: logo, banner, flyer, poster, copy.
// Presentation/slides output type has been removed; if passed it is silently stripped.
//
// Stages:
//  1. Validate      — required fields
//  2. Sanitise      — strip HTML, truncate, reject PII / spam
//  3. Classify      — industry sector, brand tone, business stage
//  4. Output flags  — which verticals are requested
//  5. Schema builder— compact JSON schema injected into prompt
...
export interface BusinessGenerateInput {
  ...
  audience?: string
  tone?: string
  outputTypes?: string[]           // 'logo' | 'banner' | 'flyer' | 'poster' | 'copy'
}
...
export async function runBusinessQCPipeline(
  ...
  // ── 4. Output type flags ───────────────────────────────────────────────────
  // 'presentation' is no longer supported — strip it if passed
  const selected   = new Set((input.outputTypes || ['logo', 'banner', 'flyer', 'poster', 'copy']).filter(t => t !== 'presentation'))
  ...
  const rules: string[] = [
    ...
    '- India relevance is mandatory when the business is Indian or no location is supplied: reflect WhatsApp-first enquiry, trust before purchase, city/local credibility, regional pricing sensitivity, Google Maps/search intent, and referral behaviour where relevant.',
    '- Create one sharp differentiator against crowded local competitors; do not output generic claims any competitor could say.',
    '- Avoid first-generation AI phrases: unlock potential, innovative solutions, game changing, seamless experience, elevate your business, one-stop solution, cutting-edge, transform your journey.',
    '- Every benefit must connect to the company, industry, audience, or acquisition channel; no vague adjectives without proof.',
    '- Prefer premium editorial/business language with practical execution: enquiry flow, trust signal, offer ladder, repeat buyers, locality, retention, WhatsApp/phone CTA.',
  ]
  ...
  if (wantPoster) rules.push(
    ...
    '- posterVisualDirection: include composition (full-bleed/editorial/centered), mood, image subject and text-safe area; avoid split panels unless explicitly requested',
  )
...
{${baseSchema}${logoSchema}${bannerSchema}${flyerSchema}${posterSchema}${copySchema}
}
