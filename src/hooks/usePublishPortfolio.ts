...
interface UsePublishPortfolioReturn {
  publish: (generationId: string, websiteTheme?: string) => Promise<PublishResult>
...
export function usePublishPortfolio(): UsePublishPortfolioReturn {
  ...
  async function publish(generationId: string, websiteTheme?: string): Promise<PublishResult> {
    setLoading(true)
    setError(null)
    try {
      const res = await fetch('/api/portfolio/publish', {
        ...
        body: JSON.stringify({ generationId, websiteTheme }),
      })
      if (!res.ok) {
