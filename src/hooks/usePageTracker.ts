  ...
  useEffect(() => {
    ...
    if (prevPath && prevPath !== pathname) {
      ...
      fetch('/api/analytics/page-visit', {
        ...
        body: JSON.stringify({ page: prevPath, durationMs, sessionId: sessionIdRef.current, timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone || '' }),
      }).catch(() => {})
    }
    ...
    fetch('/api/analytics/page-visit', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ page: pathname, durationMs: null, sessionId: sessionIdRef.current, timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone || '' }),
    }).catch(() => {})
    ...
    const handleUnload = () => {
      ...
      const payload = JSON.stringify({ page: pathname, durationMs, sessionId: sessionIdRef.current, timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone || '' })
