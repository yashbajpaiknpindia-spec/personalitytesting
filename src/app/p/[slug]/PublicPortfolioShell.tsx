...
// Per-theme CSS variable overrides applied on top of base styles
function getThemeVars(theme: string | null | undefined, accent: string, aRgb: string): string {
  const t = theme || 'the-manifesto'
  const themes: Record<string, string> = {
    'the-manifesto':  `--bg:#0A0A0B;--s:#111113;--s2:#17171A;--cr:#F0EAE0;--mu:#908070;--mu2:#605040;--bd:rgba(255,255,255,0.06);--bd2:rgba(255,255,255,0.10)`,
    'editorial-dark': `--bg:#0D0D0D;--s:#141414;--s2:#1a1a1a;--cr:#EDE8E0;--mu:#887060;--mu2:#584840;--bd:rgba(255,255,255,0.07);--bd2:rgba(255,255,255,0.11)`,
    'signal':         `--bg:#0A1520;--s:#0F1E2D;--s2:#152636;--cr:#E8F0F8;--mu:#5A8098;--mu2:#3A6078;--bd:rgba(76,168,201,0.12);--bd2:rgba(76,168,201,0.18)`,
    'founders-page':  `--bg:#111;--s:#181818;--s2:#1f1f1f;--cr:#EDE6DA;--mu:#906840;--mu2:#604828;--bd:rgba(255,255,255,0.07);--bd2:rgba(255,255,255,0.12)`,
    'zero-noise':     `--bg:#FAFAFA;--s:#F4F4F4;--s2:#EEEEEE;--cr:#111111;--mu:#666666;--mu2:#999999;--bd:rgba(0,0,0,0.08);--bd2:rgba(0,0,0,0.12)`,
    'luminary':       `--bg:#0C0C0A;--s:#141410;--s2:#1a1a14;--cr:#F2EDD6;--mu:#A08848;--mu2:#706028;--bd:rgba(226,197,122,0.1);--bd2:rgba(226,197,122,0.16)`,
    'the-architect':  `--bg:#111827;--s:#1a2333;--s2:#212d3d;--cr:#E5E7EB;--mu:#6B7280;--mu2:#4B5563;--bd:rgba(255,255,255,0.08);--bd2:rgba(255,255,255,0.14)`,
    'studio-brief':   `--bg:#0A0000;--s:#140808;--s2:#1e1010;--cr:#F5E8E8;--mu:#C05050;--mu2:#803030;--bd:rgba(192,57,43,0.14);--bd2:rgba(192,57,43,0.22)`,
    'the-operator':   `--bg:#0A1410;--s:#0F1E18;--s2:#152820;--cr:#DCF0E4;--mu:#4A8860;--mu2:#2A6040;--bd:rgba(46,125,82,0.14);--bd2:rgba(46,125,82,0.22)`,
    'case-study-pro': `--bg:#0E0B1E;--s:#160F28;--s2:#1c1430;--cr:#E8E4F8;--mu:#6A58BB;--mu2:#4A3898;--bd:rgba(123,104,238,0.14);--bd2:rgba(123,104,238,0.22)`,
    'minimal-canon':  `--bg:#F8F6F2;--s:#F0EDE6;--s2:#E8E4DC;--cr:#2A2420;--mu:#807060;--mu2:#B0A090;--bd:rgba(0,0,0,0.07);--bd2:rgba(0,0,0,0.12)`,
    'deep-work':      `--bg:#0A0A0A;--s:#111111;--s2:#181818;--cr:#E0E0E0;--mu:#808080;--mu2:#606060;--bd:rgba(255,255,255,0.06);--bd2:rgba(255,255,255,0.10)`,
  }
  const vars = themes[t] || themes['the-manifesto']
  // Light themes need dark text in overlays
  const isLight = t === 'zero-noise' || t === 'minimal-canon'
  const overlayBg = isLight
    ? `linear-gradient(to top, rgba(250,250,250,1) 0%, rgba(250,250,250,.75) 40%, rgba(250,250,250,.35) 70%, transparent 100%)`
    : `linear-gradient(to top, rgba(9,9,10,1) 0%, rgba(9,9,10,.75) 40%, rgba(9,9,10,.35) 70%, rgba(9,9,10,.2) 100%)`
  const heroTextColor = isLight ? '#111' : '#fff'
  return `:root{--a:${accent};--ar:${aRgb};${vars};--hero-overlay:${overlayBg};--hero-text:${heroTextColor}}`
}
...
export default function PublicPortfolioShell({ user, out, slug, ownerId, projects = [], blogPosts = [], username, websiteTheme, googleAnalyticsId }: Props) {
  ...
  // Fetch AI-generated images on mount (via /api/image → gpt-image-1)
  useEffect(() => {
    const load = async () => {
  ...
  // Inject GA4 gtag if the portfolio owner has configured a Measurement ID
  useEffect(() => {
  ...
  const CSS = `
    *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
    ${getThemeVars(websiteTheme, accent, aRgb)}
    ...
    .hero-overlay{position:absolute;inset:0;background:var(--hero-overlay),linear-gradient(to right, rgba(9,9,10,.6) 0%, transparent 60%)}
    ...
    .htitle{font-family:'Playfair Display',serif;font-size:clamp(40px,5.5vw,76px);color:var(--hero-text);line-height:1.04;margin-bottom:22px;font-weight:400;letter-spacing:-.015em;text-shadow:0 2px 40px rgba(0,0,0,.4)}
    .htitle em{font-style:italic;color:var(--a)}
    .hbio{font-size:15px;color:var(--cr);opacity:.75;line-height:1.85;max-width:500px;font-weight:300;margin-bottom:40px;text-shadow:0 1px 12px rgba(0,0,0,.5)}
        ...
        <div className="hero">
          <div className="hero-img-wrap">
            {heroImg
              ...
              ? <img src={heroImg} alt="" className="hero-img" loading="eager" decoding="async" onLoad={() => setHeroLoaded(true)} />
          ...
          <div className="havatarwrap">
            <div className="havaatar">
              {user.image
                // eslint-disable-next-line @next/next/no-img-element
                ? <img src={user.image} alt={user.name || ''} loading="lazy" decoding="async" />
            ...
            <div className={`wgrid${out.portfolioSections.length > 2 ? ' hf' : ''}`}>
              {out.portfolioSections.map((sec: {title:string;body:string;highlight:string}, i: number) => (
                <div key={i} className="wcard">
                  {workImgs[i]
                    // eslint-disable-next-line @next/next/no-img-element
                    ? <img src={workImgs[i]!} alt="" className="wcard-img" loading="lazy" decoding="async" />
