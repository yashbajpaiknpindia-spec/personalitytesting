...
export default async function TemplatesPage() {
  ...
  return (
    <div className="page-pad">
      ...
      <p style={{ fontSize:13, color:'var(--muted)', marginBottom:48, lineHeight:1.6 }}>Choose your aesthetic. Every template is precision-tuned for AI generation.</p>

      {(['portfolio','resume','card','presentation'] as const).map(cat => {
        const catT = templates.filter((t: { category: string }) => t.category === cat)
          ...
          <div key={cat} style={{ marginBottom:60 }}>
            <div style={{ display:'flex', alignItems:'center', gap:14, marginBottom:28 }}>
              <div style={{ width:40, height:40, borderRadius:10, background:`${meta.color}12`, border:`1px solid ${meta.color}28`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                <CatIcon cat={cat} color={meta.color}/>
              </div>
              <div>
                <div style={{ fontFamily:"'Playfair Display',serif", fontSize:20, color:'var(--cream)', marginBottom:3 }}>{cat.charAt(0).toUpperCase()+cat.slice(1)}</div>
                <div style={{ fontSize:11, color:'var(--muted)', fontFamily:"'DM Mono',monospace" }}>{meta.desc}<span style={{ marginLeft:6, color:meta.color }}>· {catT.length} templates</span></div>
              </div>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(185px,1fr))', gap:18 }}>
              {catT.map((t: { id:string; name:string; description:string|null; accentColor:string; tier:string; slug:string }) => {
                const locked = t.tier==='pro' && !isPro
                return (
                  <Link key={t.id} href={locked?'/billing':`/generate?template=${t.slug}`} style={{ textDecoration:'none' }}>
                    <div style={{ background:'var(--surface)', border:`1px solid ${locked?'rgba(201,168,76,0.18)':'var(--border)'}`, borderRadius:6, overflow:'hidden', cursor:'pointer', position:'relative', transition:'transform 0.18s,box-shadow 0.18s,border-color 0.18s' }}
                    
                    >
                      {t.tier==='pro' && (
                        <div style={{ position:'absolute', top:10, right:10, background:locked?'rgba(0,0,0,0.7)':'var(--gold)', color:locked?'var(--gold)':'#000', fontSize:8, padding:'2px 7px', letterSpacing:'0.12em', fontFamily:"'DM Mono',monospace", zIndex:2, borderRadius:2, border:locked?'1px solid rgba(201,168,76,0.38)':'none', display:'flex', alignItems:'center', gap:4 }}>
                          {locked && <LockSVG size={10} color="var(--gold)"/>}PRO
                        </div>
                      )}
                      <div style={{ height:128, background:'#09090c', position:'relative', overflow:'hidden' }}>
                        <Thumb c={t.accentColor} cat={cat}/>
                        {locked && (
                          <div style={{ position:'absolute', inset:0, background:'rgba(0,0,0,0.52)', backdropFilter:'blur(1.5px)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:7 }}>
                            <LockSVG size={26} color="var(--gold)"/>
                            <span style={{ fontSize:9, letterSpacing:'0.14em', textTransform:'uppercase', color:'var(--gold)', fontFamily:"'DM Mono',monospace" }}>Pro Only</span>
                          </div>
                        )}
                      ...
                      <div style={{ padding:'13px 15px' }}>
                        ...
                        <div style={{ fontSize:11, color:'var(--muted)', lineHeight:1.55, marginBottom:11, display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden' }}>{t.description??''}</div>
                        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                          ...
                          <span style={{ fontSize:9, color:t.tier==='pro'?'var(--gold)':'var(--muted2)', letterSpacing:'0.08em', textTransform:'uppercase', fontFamily:"'DM Mono',monospace", border:`1px solid ${t.tier==='pro'?'#C9A84C40':'var(--border)'}`, padding:'1px 6px', borderRadius:2 }}>{t.tier}</span>
                        </div>
                      </div>
                    </div>
                  </Link>
