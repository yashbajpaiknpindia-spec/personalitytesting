...
const navItems = [
  { href: '/generate/personal', label: 'Generate',   icon: 'M1 1h5v5H1zM8 1h5v5H8zM1 8h5v5H1zM8 8h5v5H8z' },
  ...
  { href: '/portfolio',      label: 'Portfolio',     icon: 'M1 1h12v4H1zM1 7h5v5H1zM8 7h5v5H8z' },
  { href: '/presentations',  label: 'Slide Decks',   icon: 'M1 1h12v8H1zM4 11h6M1 4h12M1 7h12' },
  { href: '/resume/edit',    label: 'Resume',        icon: 'M2 1h10v12H2zM5 4h4M5 6h4M5 8h4M5 10h2' },
...
const businessNavItems = [
  { href: '/generate/business', label: '✦ Generate', icon: 'M1 1h5v5H1zM8 1h5v5H8zM1 8h5v5H1zM8 8h5v5H8z', bizOnly: false },
...
function RecentGenerationsPanel() {
  ...
  const [recents, setRecents] = React.useState<Array<{
    id: string; createdAt: string; inputData?: Record<string, unknown> | null; outputData?: Record<string, unknown> | null; template?: { name: string; category: string } | null
  }>>([])
  const [loading, setLoading] = React.useState(false)

  React.useEffect(() => {
    setLoading(true)
    fetch('/api/my-generations')
      .then(r => r.ok ? r.json() : null)
      .then(data => {
        if (data?.generations) setRecents(data.generations.slice(0, 5))
      })
      .catch(() => {})
      .finally(() => setLoading(false))
  }, [])

  if (recents.length === 0 && !loading) return null

  return (
    <div style={{ margin: '0 0 0 0' }}>
      <button
        onClick={() => setOpen(o => !o)}
        style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '0 20px', fontSize: 9, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--muted)', fontFamily: "'DM Mono', monospace", background: 'transparent', border: 'none', cursor: 'pointer', marginBottom: open ? 8 : 0 }}
      >
        <span>Recent</span>
        <span style={{ fontSize: 11, opacity: 0.5, transition: 'transform 0.2s', display: 'inline-block', transform: open ? 'rotate(180deg)' : 'rotate(0deg)' }}>▾</span>
      </button>
      {open && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, padding: '0 12px' }}>
          {loading ? (
            <div style={{ padding: '8px 8px', fontSize: 10, color: 'var(--muted)', fontFamily: "'DM Mono', monospace" }}>Loading…</div>
          ) : recents.map(gen => {
            const inp = gen.inputData as Record<string, string> | null
            const out = gen.outputData as Record<string, string> | null
            const isBusiness = !!(inp?.companyName && inp.companyName.trim().length > 0)
            const modeLabel = isBusiness ? '✦ Business' : '⬡ Personal'
            const rawSnippet = inp?.name || inp?.companyName || out?.cardName || out?.companyName || 'Generation'
            const snippet = rawSnippet.slice(0, 26) + (rawSnippet.length > 26 ? '…' : '')
            const href = isBusiness
              ? `/generate/business?gen=${gen.id}`
              : `/generate/personal?from=${gen.id}&preview=1`
            const date = new Date(gen.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })
            return (
              <a key={gen.id} href={href} style={{ display: 'flex', flexDirection: 'column', gap: 3, padding: '8px 8px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', textDecoration: 'none', transition: 'border-color 0.15s' }}
                onMouseEnter={e => (e.currentTarget as HTMLElement).style.borderColor = 'var(--gold)'}
                onMouseLeave={e => (e.currentTarget as HTMLElement).style.borderColor = 'var(--border)'}
              >
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 }}>
                  <span style={{ fontSize: 8, color: 'var(--gold)', fontFamily: "'DM Mono', monospace", letterSpacing: '0.08em' }}>{modeLabel}</span>
                  <span style={{ fontSize: 8, color: 'var(--muted2)', fontFamily: "'DM Mono', monospace" }}>{date}</span>
                </div>
                <div style={{ fontSize: 11, color: 'var(--cream)', lineHeight: 1.4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{snippet}</div>
                <div style={{ fontSize: 9, color: 'var(--gold)', fontFamily: "'DM Mono', monospace", letterSpacing: '0.06em' }}>Open →</div>
              </a>
            )
          })}
        </div>
...
export default function AppShell({ children, session }: Props) {
  ...
  const isBusinessMode = !!(session && (pathname.startsWith('/business') || pathname.startsWith('/generate/business')))
  ...
  const topNavLinks = session
    ? isBusinessMode
      ? [
          { href: '/generate/business', label: 'Generate' },
          { href: '/business/edit', label: 'Brand Assets' },
          { href: '/business/edit?tab=copy', label: 'Campaigns' },
          { href: '/my-work', label: 'History' },
        ]
      : [{ href: '/generate', label: 'Generate' }, { href: '/templates', label: 'Templates' }, { href: '/my-work', label: 'My Work' }, { href: '/portfolio', label: 'Portfolio' }, { href: '/presentations', label: 'Slide Decks' }]
    ...
    <>
      ...
      {isBusinessMode && session && (
        <div style={{ margin: '0 20px 12px', padding: '10px 14px', background: 'rgba(201,168,76,0.1)', border: '1px solid rgba(201,168,76,0.35)', borderRadius: 'var(--radius)' }}>
          ...
          <Link href="/generate/personal" style={{ display: 'inline-block', marginTop: 8, fontSize: 9, color: 'var(--muted)', fontFamily: "'DM Mono', monospace", textDecoration: 'none', letterSpacing: '0.08em' }}>← Switch to Personal</Link>
        </div>
      )}
  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      ...
      <nav className="top-nav" style={{
        ...
        {/* Mode Switcher Pill — desktop, always visible when authed */}
        {session && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 0, background: 'var(--surface)', border: '1px solid var(--border2)', borderRadius: 'var(--radius)', padding: 2 }}>
            <a href="/generate/personal" style={{ padding: '4px 12px', fontSize: 9, letterSpacing: '0.1em', textTransform: 'uppercase', fontFamily: "'DM Mono', monospace", cursor: 'pointer', background: !isBusinessMode ? 'var(--gold)' : 'transparent', color: !isBusinessMode ? '#000' : 'var(--muted)', border: 'none', borderRadius: 2, fontWeight: !isBusinessMode ? 600 : 400, transition: 'all 0.15s', textDecoration: 'none', display: 'inline-block' }}>Personal</a>
            <a href="/generate/business" style={{ padding: '4px 12px', fontSize: 9, letterSpacing: '0.1em', textTransform: 'uppercase', fontFamily: "'DM Mono', monospace", cursor: 'pointer', background: isBusinessMode ? 'var(--gold)' : 'transparent', color: isBusinessMode ? '#000' : 'var(--muted)', border: 'none', borderRadius: 2, fontWeight: isBusinessMode ? 600 : 400, transition: 'all 0.15s', textDecoration: 'none', display: 'inline-block' }}>Business ✦</a>
          </div>
        )}
