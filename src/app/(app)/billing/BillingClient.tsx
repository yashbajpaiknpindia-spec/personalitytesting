...
const PLANS = [
  {
    ...
    features: ['3 generations per month', '12 free templates', 'PDF export (watermarked)', 'Public portfolio URL', 'QR code generation'],
  },
  {
    id: 'PRO', name: 'Pro', price: '$19', period: '/month',
    features: ['Unlimited generations', 'All 48 templates', 'PDF, PPTX, vCard export', 'No watermarks', 'Version history', 'Priority processing', 'Analytics dashboard'],
  },
  {
    id: 'TEAM', name: 'Team', price: '$49', period: '/month',
    features: ['Everything in Pro', 'Up to 5 team members', 'Shared template library', 'Brand kit management', 'Admin dashboard', 'Priority support'],
  },
]
...
function BillingContent({ plan }: { plan: string }) {
  ...
  // ─── ONLY CHANGED: upgrade() and managePortal() internals ──────────────────
    ...
    <div style={{ padding: '40px 36px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 16, marginBottom: 40 }}>
        {PLANS.map(p => {
          ...
          const isUpgrade = p.id !== 'FREE' && p.id !== plan
          return (
            <div key={p.id} style={{ background: 'var(--surface)', border: `1px solid ${isCurrent ? 'var(--gold)' : 'var(--border)'}`, borderRadius: 'var(--radius)', padding: 24, position: 'relative' }}>
              ...
              <div style={{ fontSize: 32, fontFamily: "'Playfair Display', serif", color: 'var(--gold)', marginBottom: 20 }}>
              ...
              <div style={{ marginTop: 20 }}>
                {isCurrent && plan !== 'FREE' ? (
                  ...
                  <button onClick={() => upgrade(p.id)} disabled={loading === p.id} style={{ width: '100%', padding: '9px 0', background: 'var(--gold)', border: 'none', color: '#000', fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 500, cursor: 'pointer', borderRadius: 'var(--radius)', fontFamily: "'DM Sans', sans-serif" }}>
