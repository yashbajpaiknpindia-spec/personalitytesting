...
import AnalyticsTabs from './AnalyticsTabsLoader'
...
async function UsageContent({ userId }: { userId: string }) {
  ...
  generations.forEach((g: { createdAt: Date; templateId: string | null; template: { name: string } | null }) => {
  ...
  generations.forEach((g: { createdAt: Date; templateId: string | null; template: { name: string } | null }) => {
    ...
    if (!id) return
    if (!templateCounts[id]) templateCounts[id] = { name: g.template?.name ?? id, count: 0 }
...
export default async function AnalyticsPage() {
  ...
  return (
    <div className="page-pad">
      ...
      <h1 className="page-h1" style={{ marginBottom: 32 }}>Usage <em>dashboard.</em></h1>
