...
function SeoPanel() {
    ...
    fetch('/api/portfolio/seo').then(r => r.ok ? r.json() : null).then(d => {
      if (d?.seo) {
        setForm({
          ...
          googleAnalyticsId: d.seo.googleAnalyticsId ?? '',
          gscVerificationToken: d.seo.gscVerificationToken ?? '',
        })
        setSlug(d.seo.slug ?? null)
      } else {
        setNoPortfolio(true)
      }
  ...
  // ── SEO health checks ──────────────────────────────────────────────────
  const checks: { label: string; pass: boolean; hint: string }[] = [
    {
      label: 'Page title set',
      pass: form.seoTitle.length > 0,
      hint: 'Add a custom page title (50–60 chars ideal).',
    },
    {
      label: 'Title length OK (< 60 chars)',
      pass: form.seoTitle.length > 0 && form.seoTitle.length <= 60,
      hint: `Current: ${form.seoTitle.length} chars. Keep under 60 to avoid truncation.`,
    },
    {
      label: 'Meta description set',
      pass: form.seoDescription.length > 0,
      hint: 'Write a compelling description for search snippet (120–155 chars ideal).',
    },
    {
      label: 'Description length OK (120–155 chars)',
      pass: form.seoDescription.length >= 120 && form.seoDescription.length <= 155,
      hint: `Current: ${form.seoDescription.length} chars. Google truncates beyond ~155.`,
    },
    {
      label: 'OG image configured',
      pass: form.ogImageUrl.length > 0,
      hint: 'Add an OG image URL (1200×630px recommended) for social share previews.',
    },
    {
      label: 'Google Analytics connected',
      pass: /^G-[A-Z0-9]{6,}$/.test(form.googleAnalyticsId),
      hint: "Enter your GA4 Measurement ID (e.g. G-XXXXXXXXXX) to track visitors.",
    },
    {
      label: 'Search Console verified',
      pass: form.gscVerificationToken.length > 0,
      hint: 'Paste your GSC meta tag content to verify ownership with Google.',
    },
  ]
  const score = checks.filter(c => c.pass).length
  const scoreColor = score >= 6 ? '#2E7D52' : score >= 4 ? '#C9A84C' : '#C0392B'
  const scoreLabel = score >= 6 ? 'Excellent' : score >= 4 ? 'Good' : 'Needs Work'
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 24 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 20, color: 'var(--cream)' }}>Portfolio SEO Score</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 32, color: scoreColor, lineHeight: 1 }}>{score}<span style={{ fontSize: 16, color: 'var(--muted)' }}>/{checks.length}</span></div>
            <div style={{ fontSize: 10, color: scoreColor, fontFamily: mono, letterSpacing: '0.1em', textTransform: 'uppercase' }}>{scoreLabel}</div>
          </div>
        </div>
        {/* Progress bar */}
        <div style={{ height: 4, background: 'var(--border)', borderRadius: 2, marginBottom: 20, overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${(score / checks.length) * 100}%`, background: scoreColor, borderRadius: 2, transition: 'width 0.4s ease' }} />
        </div>
        {/* Checklist */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {checks.map((c, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '8px 12px', background: c.pass ? 'rgba(46,125,82,0.06)' : 'rgba(192,57,43,0.05)', borderRadius: 4, border: `1px solid ${c.pass ? 'rgba(46,125,82,0.2)' : 'rgba(192,57,43,0.15)'}` }}>
              <div style={{ fontSize: 13, color: c.pass ? '#6FCF97' : '#E57373', flexShrink: 0, marginTop: 1 }}>{c.pass ? '✓' : '✕'}</div>
              <div>
                <div style={{ fontSize: 12, color: c.pass ? '#6FCF97' : 'var(--cream)', fontFamily: mono, letterSpacing: '0.04em' }}>{c.label}</div>
                {!c.pass && <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 2, lineHeight: 1.5 }}>{c.hint}</div>}
              </div>
            </div>
          ))}
        </div>
      ...
      {/* ── Basic SEO ────────────────────────────────────────────────── */}
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 24 }}>
        <SL label="Search Appearance" />
        <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 18, color: 'var(--cream)', marginBottom: 6 }}>Meta Tags</div>
        <p style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 20, lineHeight: 1.7 }}>
          These override your generation&apos;s defaults in search results and social previews.
          {noPortfolio && <><br /><span style={{ color: '#e07040' }}>⚠ Publish your portfolio first, then set SEO.</span></>}
        </p>
        <div style={{ marginBottom: 14 }}>
          <div style={ls}>Page Title <span style={{ color: form.seoTitle.length > 60 ? '#E57373' : 'var(--muted2)' }}>({form.seoTitle.length}/60)</span></div>
          <input value={form.seoTitle} onChange={e => setForm(f => ({ ...f, seoTitle: e.target.value }))} style={fs} placeholder="My Portfolio | Your Name" />
        </div>
        <div style={{ marginBottom: 14 }}>
          <div style={ls}>Meta Description <span style={{ color: form.seoDescription.length > 155 ? '#E57373' : form.seoDescription.length >= 120 ? '#6FCF97' : 'var(--muted2)' }}>({form.seoDescription.length}/155)</span></div>
          <textarea value={form.seoDescription} onChange={e => setForm(f => ({ ...f, seoDescription: e.target.value }))} rows={3} style={{ ...fs, resize: 'vertical' as const }} placeholder="A compelling description of your portfolio…" />
        </div>
        <div style={{ marginBottom: 0 }}>
          <div style={ls}>OG Image URL <span style={{ color: 'var(--muted2)', fontSize: 10 }}>Recommended: 1200×630px</span></div>
          <input value={form.ogImageUrl} onChange={e => setForm(f => ({ ...f, ogImageUrl: e.target.value }))} style={fs} placeholder="https://res.cloudinary.com/…" />
        </div>
      </div>

      {/* ── Google Analytics 4 ───────────────────────────────────────── */}
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 24 }}>
        <SL label="Visitor Analytics" />
        <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 18, color: 'var(--cream)', marginBottom: 6 }}>Google Analytics 4</div>
        <p style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 20, lineHeight: 1.7 }}>
          Connect your own GA4 property to track who visits your portfolio.
          Find your Measurement ID in <span style={{ color: 'var(--gold)', fontFamily: mono }}>GA4 → Admin → Data Streams → your stream</span>.
        </p>
        <div style={{ marginBottom: 0 }}>
          <div style={ls}>Measurement ID</div>
          <input
          ...
          />
          {form.googleAnalyticsId && !/^G-[A-Z0-9]{6,}$/.test(form.googleAnalyticsId) && (
            <div style={{ fontSize: 11, color: '#E57373', marginTop: 4 }}>⚠ Format should be G- followed by letters/numbers</div>
          )}
          {/^G-[A-Z0-9]{6,}$/.test(form.googleAnalyticsId) && (
            <div style={{ fontSize: 11, color: '#6FCF97', marginTop: 4 }}>✓ Valid Measurement ID, will be injected into your portfolio page</div>
          )}
        </div>
      </div>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 24 }}>
        <SL label="Ownership Verification" />
        <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 18, color: 'var(--cream)', marginBottom: 6 }}>Google Search Console</div>
        <p style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 20, lineHeight: 1.7 }}>
          Verify ownership to submit your sitemap to Google and monitor search performance.<br />
          In GSC: <span style={{ color: 'var(--gold)', fontFamily: mono }}>Add Property → URL prefix → HTML tag method</span>. Copy only the <em>content=&quot;…&quot;</em> value, not the full tag.
        </p>
        <div style={{ marginBottom: 0 }}>
          <div style={ls}>Verification token (content= value only)</div>
          <input
            value={form.gscVerificationToken}
            onChange={e => setForm(f => ({ ...f, gscVerificationToken: e.target.value }))}
            style={fs}
            placeholder="AbCdEfGhIjKlMnOpQrStUvWxYz01234567890abcde"
          />
          <div style={{ fontSize: 11, color: 'var(--muted2)', marginTop: 6, lineHeight: 1.6 }}>
            This becomes: <span style={{ fontFamily: mono, color: 'var(--muted)', fontSize: 10 }}>&lt;meta name=&quot;google-site-verification&quot; content=&quot;{form.gscVerificationToken || '…'}&quot; /&gt;</span>
          </div>
        </div>

        {/* Sitemap URL for GSC */}
        {slug && (
          <div style={{ marginTop: 20, background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '14px 16px' }}>
            <div style={{ fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--gold)', fontFamily: mono, marginBottom: 8 }}>Your Portfolio Sitemap URL</div>
            <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 8, lineHeight: 1.6 }}>
              Submit this URL in GSC → Sitemaps to help Google index your portfolio pages faster.
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <code style={{ fontSize: 11, fontFamily: mono, color: 'var(--cream)', background: 'var(--surface)', border: '1px solid var(--border2)', padding: '6px 10px', borderRadius: 3, flex: 1, wordBreak: 'break-all' as const }}>
                {BASE}/api/sitemap/{slug}
              </code>
              <button
                onClick={() => navigator.clipboard.writeText(`${BASE}/api/sitemap/${slug}`)}
                style={{ background: 'transparent', border: '1px solid var(--border2)', color: 'var(--muted)', padding: '6px 10px', fontSize: 10, cursor: 'pointer', borderRadius: 3, fontFamily: mono, whiteSpace: 'nowrap' as const, flexShrink: 0 }}
              >
                Copy
              </button>
              <a href={`/api/sitemap/${slug}`} target="_blank" rel="noopener noreferrer" style={{ background: 'transparent', border: '1px solid rgba(201,168,76,0.4)', color: 'var(--gold)', padding: '6px 10px', fontSize: 10, cursor: 'pointer', borderRadius: 3, fontFamily: mono, textDecoration: 'none', whiteSpace: 'nowrap' as const, flexShrink: 0 }}>
                Preview ↗
              </a>
            </div>
          </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <button
          onClick={save}
          disabled={loading || noPortfolio}
          style={{
            background: 'var(--gold)', color: '#000', border: 'none',
            padding: '11px 32px', fontSize: 11, letterSpacing: '0.12em',
            textTransform: 'uppercase', fontFamily: mono, fontWeight: 500,
            cursor: (loading || noPortfolio) ? 'default' : 'pointer',
            borderRadius: 'var(--radius)', opacity: (loading || noPortfolio) ? 0.5 : 1,
            transition: 'opacity 0.2s',
          }}
        >
          {loading ? 'Saving…' : saved ? '✓ Saved' : 'Save All SEO Settings'}
        </button>
        {saved && <span style={{ fontSize: 11, color: '#6FCF97', fontFamily: mono }}>Changes live on your portfolio</span>}
        {noPortfolio && <span style={{ fontSize: 11, color: '#e07040', fontFamily: mono }}>Publish your portfolio first</span>}
      </div>
    </div>
  )
}
...
function ProfileSectionsPanel({ accent }:{ accent:string }) {
  ...
  return (
    <div>
      {toast && <div style={{ position:'fixed', bottom:24, right:24, background:'var(--surface)', border:`1px solid ${accent}60`, borderRadius:6, padding:'10px 18px', fontSize:12, color:'#F0EAE0', zIndex:9999 }}>{toast}</div>}
      <div style={sec}>
