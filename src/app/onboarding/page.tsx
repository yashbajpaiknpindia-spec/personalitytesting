...
const ACCENTS = ['#C9A84C', '#E2C57A', '#4CA8C9', '#7B68EE', '#C0392B', '#2E7D52', '#F0EAE0', '#B85C2A']
...
export default function OnboardingPage() {
  ...
  async function finish() {
    ...
    await fetch('/api/user/profile', {
      ...
      body: JSON.stringify({ accentColor: accent, onboarded: true, jobTitle: role }),
    })
    await update({ onboarded: true })
    // BUG FIX #8: Hard navigation so middleware sees the fresh onboarded token
