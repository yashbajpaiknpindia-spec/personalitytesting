...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    dailyUsageCount: (user as any).dailyUsageCount,
    dailyUsageDate: (user as any).dailyUsageDate,
    dailyGenLimit: (user as any).dailyGenLimit,
    monthlyGenLimit: (user as any).monthlyGenLimit,
    yearlyGenLimit: (user as any).yearlyGenLimit,
  })
}
  ...
  try {
    ...
    if (body.email !== undefined) {
      ...
      if (email) {
        ...
        if (existingEmail) return NextResponse.json({ error: 'Email already used by another account' }, { status: 409 })
      }
    }
