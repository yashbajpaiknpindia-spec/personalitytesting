...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    posterEditLimit: (s as any)?.posterEditLimit ?? 2,
    // ── Master generation limit ────────────────────────────────────────────
    globalGenLimit:    (s as any)?.globalGenLimit    ?? 5,
    globalLimitPeriod: (s as any)?.globalLimitPeriod ?? 'daily',
    // ── Per-plan fine-grained limits ───────────────────────────────────────
    limits: {
...
export async function POST(req: NextRequest) {
  ...
  // Poster edit limit
  if (body.posterEditLimit !== undefined) {
    const limit = Math.max(0, Math.floor(Number(body.posterEditLimit)))
    if (!isNaN(limit)) updateData.posterEditLimit = limit
  }

  // ── Master generation limit ──────────────────────────────────────────────
  if (body.globalGenLimit !== undefined) {
    const gl = Math.max(0, Math.floor(Number(body.globalGenLimit)))
    if (!isNaN(gl)) updateData.globalGenLimit = gl
  }
  if (body.globalLimitPeriod !== undefined) {
    const period = String(body.globalLimitPeriod)
    if (period === 'daily' || period === 'monthly') updateData.globalLimitPeriod = period
  }
  ...
  return NextResponse.json({
    ...
    posterEditLimit:    (updated as any).posterEditLimit    ?? 2,
    globalGenLimit:     (updated as any).globalGenLimit     ?? 5,
    globalLimitPeriod:  (updated as any).globalLimitPeriod  ?? 'daily',
    limits: {
